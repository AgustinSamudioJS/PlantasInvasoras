...
public class Herramientas {
	...
	/**
	 * Levanta una imagen desde el archivo dado y devuelve un objeto de tipo Image
	 * con la misma.
	 * 
	 ...
	 * 
	 * Para ejecutar una vez el un Clip se puede utilizar la funci�n start() de este
	 * objeto. Por otro lado, para ejecutar el sonido en f�rma c�clica se puede
	 * utilizar la funci�n loop() de este objeto. La cual toma como par�metro la
	 * cantidad de ejecuciones a realizar (para ciclar infinitamente usar
	...
	public static void MatrizSinNull(String puntaje[][]) {
	...
	public static void AgregarUsuario(String puntaje[][], String usuario,String puntos) {
		for (int i = 0; i < puntaje.length; i++) {
			for (int j = 0; j < puntaje[i].length; j++) {
				if (puntaje[i][j] == "") {
					puntaje[i][j] = usuario;
					puntaje[i][j+1]=puntos;
					return;
				}
			}
			System.out.println(); // Imprime salto de línea
	...
	// MOSTRAR LA MATRIZ
	public static void ImprimirMatriz(String puntaje[][]) {
		for (int i = 0; i < puntaje.length; i++) {
			for (int j = 0; j < puntaje[i].length; j++) {
